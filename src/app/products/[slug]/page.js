import { 
  Zap, 
  Shield, 
  Users, 
  BarChart3, 
  Clock, 
  MessageSquare, 
  TrendingUp, 
  Cloud 
} from "lucide-react";
import Image from "next/image";

export default function App() {
  return (
    <div className="min-h-screen bg-white mt-20">
      {/* Section 1 - Vertical Product Introduction Panel */}
      <section className="flex flex-col lg:flex-row min-h-[600px]">
        {/* Left Panel - 30% */}
        <div className="lg:w-[30%] bg-[#f5f7f9] p-8 lg:p-12 flex flex-col justify-center relative">
          <div className="space-y-6">
            <p className="text-xs tracking-[0.2em] text-[#6b7280] uppercase font-medium">
              Product
            </p>
            <h1 className="text-4xl lg:text-5xl font-light leading-tight text-[#111827]">
              Enterprise
              <br />
              <span className="text-[#14b8a6]">Collaboration</span>
              <br />
              Platform
            </h1>
            <div className="w-16 h-[2px] bg-[#14b8a6]"></div>
            <p className="text-[#4b5563] leading-relaxed max-w-sm">
              Streamline workflows and empower teams with intelligent automation and real-time collaboration.
            </p>
          </div>
          {/* Divider line */}
          <div className="hidden lg:block absolute right-0 top-0 bottom-0 w-[1px] bg-[#e5e7eb]"></div>
        </div>

        {/* Right Panel - 70% */}
        <div className="lg:w-[70%] p-8 lg:p-16 flex items-center justify-center bg-white">
          <div className="max-w-3xl w-full">
            <Image
              src="/assets/img/products/Enterprise.jpeg"
              alt="Enterprise Platform"
              width={1000}
              height={1000}
              className="w-full h-auto shadow-[0_2px_20px_rgba(0,0,0,0.06)]"
            />
          </div>
        </div>
      </section>

      {/* Section 2 - Key Features (Horizontal Architecture Grid) */}
      <section className="py-16 lg:py-24 px-6 lg:px-12">
        <h2 className="text-3xl font-light text-[#111827] mb-12">Core Capabilities</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0">
          {/* Feature 1 */}
          <div className="p-8 border-r-0 lg:border-r border-[#e5e7eb] border-b md:border-b-0">
            <Zap className="w-8 h-8 text-[#14b8a6] mb-6 stroke-[1.5]" />
            <h3 className="text-lg font-semibold text-[#111827] mb-3">
              Instant Deployment
            </h3>
            <p className="text-[#6b7280] leading-relaxed text-sm">
              Launch projects in minutes with automated infrastructure provisioning.
            </p>
          </div>

          {/* Feature 2 */}
          <div className="p-8 border-r-0 lg:border-r border-[#e5e7eb] border-b md:border-b-0">
            <Shield className="w-8 h-8 text-[#14b8a6] mb-6 stroke-[1.5]" />
            <h3 className="text-lg font-semibold text-[#111827] mb-3">
              Enterprise Security
            </h3>
            <p className="text-[#6b7280] leading-relaxed text-sm">
              Bank-grade encryption and compliance with industry standards.
            </p>
          </div>

          {/* Feature 3 */}
          <div className="p-8 border-r-0 lg:border-r border-[#e5e7eb] border-b md:border-b-0">
            <Users className="w-8 h-8 text-[#14b8a6] mb-6 stroke-[1.5]" />
            <h3 className="text-lg font-semibold text-[#111827] mb-3">
              Team Collaboration
            </h3>
            <p className="text-[#6b7280] leading-relaxed text-sm">
              Work seamlessly across departments with integrated communication.
            </p>
          </div>

          {/* Feature 4 */}
          <div className="p-8">
            <BarChart3 className="w-8 h-8 text-[#14b8a6] mb-6 stroke-[1.5]" />
            <h3 className="text-lg font-semibold text-[#111827] mb-3">
              Advanced Analytics
            </h3>
            <p className="text-[#6b7280] leading-relaxed text-sm">
              Real-time insights and performance metrics at your fingertips.
            </p>
          </div>
        </div>
      </section>

      {/* Section 3 - Business Benefits (Structured Quadrant Layout) */}
      <section className="py-16 lg:py-24 bg-[#f5f7f9]">
        <div className="max-w-6xl mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
            {/* Top Left - Time Benefits */}
            <div className="p-8 lg:p-12 border-b lg:border-r border-[#e5e7eb] bg-white/50">
              <p className="text-xs tracking-[0.2em] text-[#6b7280] uppercase font-medium mb-4">
                Time Benefits
              </p>
              <div className="w-12 h-[1px] bg-[#14b8a6] mb-8"></div>
              <div className="space-y-6">
                <div className="flex items-start gap-3">
                  <Clock className="w-5 h-5 text-[#14b8a6] mt-0.5 stroke-[1.5] flex-shrink-0" />
                  <p className="text-[#4b5563] leading-relaxed">
                    Reduce project setup time by 70% with automated workflows
                  </p>
                </div>
                <div className="flex items-start gap-3">
                  <Clock className="w-5 h-5 text-[#14b8a6] mt-0.5 stroke-[1.5] flex-shrink-0" />
                  <p className="text-[#4b5563] leading-relaxed">
                    Deploy updates in seconds with continuous integration
                  </p>
                </div>
              </div>
            </div>

            {/* Top Right - Communication Benefits */}
            <div className="p-8 lg:p-12 border-b border-[#e5e7eb] bg-white/50">
              <p className="text-xs tracking-[0.2em] text-[#6b7280] uppercase font-medium mb-4">
                Communication Benefits
              </p>
              <div className="w-12 h-[1px] bg-[#06b6d4] mb-8"></div>
              <div className="space-y-6">
                <div className="flex items-start gap-3">
                  <MessageSquare className="w-5 h-5 text-[#06b6d4] mt-0.5 stroke-[1.5] flex-shrink-0" />
                  <p className="text-[#4b5563] leading-relaxed">
                    Unified messaging across all teams and departments
                  </p>
                </div>
                <div className="flex items-start gap-3">
                  <MessageSquare className="w-5 h-5 text-[#06b6d4] mt-0.5 stroke-[1.5] flex-shrink-0" />
                  <p className="text-[#4b5563] leading-relaxed">
                    Real-time notifications and activity tracking
                  </p>
                </div>
              </div>
            </div>

            {/* Bottom Left - Growth Benefits */}
            <div className="p-8 lg:p-12 lg:border-r border-[#e5e7eb] bg-white/50">
              <p className="text-xs tracking-[0.2em] text-[#6b7280] uppercase font-medium mb-4">
                Growth Benefits
              </p>
              <div className="w-12 h-[1px] bg-[#22c55e] mb-8"></div>
              <div className="space-y-6">
                <div className="flex items-start gap-3">
                  <TrendingUp className="w-5 h-5 text-[#22c55e] mt-0.5 stroke-[1.5] flex-shrink-0" />
                  <p className="text-[#4b5563] leading-relaxed">
                    Scale infrastructure automatically based on demand
                  </p>
                </div>
                <div className="flex items-start gap-3">
                  <TrendingUp className="w-5 h-5 text-[#22c55e] mt-0.5 stroke-[1.5] flex-shrink-0" />
                  <p className="text-[#4b5563] leading-relaxed">
                    Support unlimited users without performance degradation
                  </p>
                </div>
              </div>
            </div>

            {/* Bottom Right - Cloud Benefits */}
            <div className="p-8 lg:p-12 bg-white/50">
              <p className="text-xs tracking-[0.2em] text-[#6b7280] uppercase font-medium mb-4">
                Cloud Benefits
              </p>
              <div className="w-12 h-[1px] bg-[#14b8a6] mb-8"></div>
              <div className="space-y-6">
                <div className="flex items-start gap-3">
                  <Cloud className="w-5 h-5 text-[#14b8a6] mt-0.5 stroke-[1.5] flex-shrink-0" />
                  <p className="text-[#4b5563] leading-relaxed">
                    99.9% uptime with redundant global infrastructure
                  </p>
                </div>
                <div className="flex items-start gap-3">
                  <Cloud className="w-5 h-5 text-[#14b8a6] mt-0.5 stroke-[1.5] flex-shrink-0" />
                  <p className="text-[#4b5563] leading-relaxed">
                    Automatic backups and disaster recovery built-in
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Section 4 - Editorial Highlight Section */}
      <section className="py-16 lg:py-24 border-t border-b border-[#e5e7eb]">
        <div className="max-w-6xl mx-auto px-6 lg:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            <div>
              <h2 className="text-3xl lg:text-4xl font-light text-[#111827] leading-tight">
                Transform how your organization builds, collaborates, and delivers.
              </h2>
            </div>
            <div className="flex items-center">
              <p className="text-[#4b5563] leading-relaxed text-lg">
                Our platform brings together the tools your team needs to work efficiently. 
                From ideation to deployment, streamline every step of your workflow with 
                intelligent automation and enterprise-grade infrastructure.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* Section 5 - Final CTA Strip */}
      <section className="py-20 lg:py-28 bg-white">
        <div className="max-w-2xl mx-auto px-6 text-center">
          <h2 className="text-3xl lg:text-4xl font-light text-[#111827] mb-4">
            Ready to get started?
          </h2>
          <p className="text-[#6b7280] mb-8 text-lg">
            Join thousands of teams already transforming their workflow.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button className="px-8 py-3 bg-[#14b8a6] text-white font-medium hover:bg-[#0d9488] transition-colors">
              Request Demo
            </button>
            <a href="#" className="text-[#14b8a6] font-medium hover:text-[#0d9488] transition-colors">
              View Documentation →
            </a>
          </div>
        </div>
      </section>
    </div>
  );
}
